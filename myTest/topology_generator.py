...
if __name__ == "__main__":
    N_MZM, N_DET = 3, 4
    n_sample = 1000  # 设定你需要收集的样本总数
    topologies = []
    unique_hashes = set()  # 用于确保收集到的拓扑不重复
    attempts = 0

    print(f"开始随机搜索，目标收集 {n_sample} 个不重复的合法拓扑...")
    # 增加最大尝试次数，因为越往后随机到新结构的概率越低
    while len(topologies) < n_sample and attempts < 1000000:
        attempts += 1
        ...
        # 去重检查：将矩阵展平为 tuple 进行哈希比对
        a_tuple = tuple(A.flatten())
        if a_tuple not in unique_hashes:
            unique_hashes.add(a_tuple)
            topologies.append(A.copy())

        # 进度提示
        if len(topologies) % 100 == 0 and len(topologies) > 0 and attempts % 10 == 0:
            print(f"已收集 {len(topologies)} 个独特拓扑 (尝试次数: {attempts})...")

    n_total = len(topologies)
    print(f"搜索结束！共进行了 {attempts} 次尝试，成功生成 {n_total} 个独特的有效拓扑。")

    if n_total > 0:
        save_dir = "random_topology_1000"
        ...
        print(f"正在将图像以 3x3 格式保存至 '{save_dir}' 文件夹...")
        for f_idx in range(n_figs):
            ...
            for i in range(n_per_fig):
                ...
                if topo_idx < n_total:
                    visualize_topology(ax, topologies[topo_idx], N_MZM, N_DET, title=f"Sample {topo_idx + 1}")
                else:
                    ax.axis('off')

            plt.tight_layout()
            file_path = os.path.join(save_dir, f"random_batch_{f_idx + 1}.png")
