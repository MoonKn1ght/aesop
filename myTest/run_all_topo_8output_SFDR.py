...
class Local_SFDREvaluator(Evaluator):
    def __init__(self, propagator, targets, evaluation_nodes):
        super().__init__()
        self.propagator = propagator
        self.targets = targets
        ...
        for nid, target_field in self.targets.items():
            ...
            self.target_norms[nid] = norm_tgt
            self.masks[nid] = np.squeeze((norm_tgt < 0.05).astype(float))
            self.peak_indices[nid] = np.argmax(norm_tgt)
    ...
    def evaluate(self, graph):
        ...
        for node in self.evaluation_nodes:
            ...
            if signal is None: return 10000.0
            ...
            time_cost += (shape_error + penalty)
            idx = self.peak_indices[node]
            recovered_signal.append(p_signal[idx])

        time_cost = time_cost * 100.0
        ...
        recovered_signal = recovered_signal - np.mean(recovered_signal)

        spectrum = np.abs(np.fft.fft(recovered_signal))
        signal_power = spectrum[1] ** 2 + 1e-15
        spur_power = np.max(spectrum[2:5]) ** 2 + 1e-15

        sjr_db = 10 * np.log10(signal_power / spur_power)
        target_sjr = 35.0
        sjr_penalty = max(0, target_sjr - sjr_db) * 1.0

        total_cost = time_cost + sjr_penalty
        return total_cost
...
def build_graph_from_matrix(A, input_laser_template, N_mzm, N_det):
    ...
    for i in range(1, 1 + N_mzm):
        ...
        # ====== 【神级物理降维】 ======
        # 根据当前 MZM 所在的拓扑层数，自动推算其应该具备的理论最佳工作频率
        lvl = levels[i]
        # 防止极端深度的拓扑频率低于 1.25GHz (最多到第3层)
        eff_lvl = min(lvl, 3)
        # 层级计算：Level 1 -> 5GHz, Level 2 -> 2.5GHz, Level 3 -> 1.25GHz
        ideal_f_rf = 10.0e9 / (2 ** eff_lvl)

        # 强行赋予理论频率，并保持参数锁定 (算法不会再瞎改这个频率了)
        mzm.default_parameters[3] = ideal_f_rf
        mzm.parameters = copy.deepcopy(mzm.default_parameters)
        # ===============================
...
if __name__ == '__main__':
    ...
    # 顺应物理二叉树的自然解复用顺序（位反转序列 1, 5, 3, 7, 2, 6, 4, 8）
    ...
    # evaluator = Local_SFDREvaluator(propagator, targets=target_ids_map, evaluation_nodes=eval_ids)
    ...
    # 如果想跑全量测试，改回 MAX_TO_OPTIMIZE = n_topos
    ...
    for rank in range(top_k):
        ...
        for nid, node_data in opt_graph.nodes.items():
            ...
            if node_obj and hasattr(node_obj, 'node_acronym') and 'MZM' in node_obj.node_acronym:
                ...
                v_bias = float(np.array(p[1]).flatten()[0])
                v_rf = float(np.array(p[2]).flatten()[0])
                # 直接展示算法找出的连续频率，让你看清楚是不是收敛到了 5G/2.5G
                k_mult = float(np.array(p[3]).flatten()[0])
                f_rf_ghz = float(np.array(p[3]).flatten()[0]) / 1e9
                phase = float(np.array(p[4]).flatten()[0])

                param_str_lines.append(
                    f"{node_obj.node_acronym}: Vb={v_bias:.2f}V, Vr={v_rf:.2f}V, f={f_rf_ghz:.2f}GHz, Ph={phase:.2f}rad"
                )

        # 2. 提取延迟线参数 (必须是 'DL' 才处理，防止读到 laser 报错)
    ...
    for rank in range(top_k):
        ...
        top_100_export_data.append({
            'rank': rank + 1, 'id': res['id'], 'score': res['score'],
            'matrix': res['matrix'], 'raw_params': res['params'],
            'node_params': node_params, 'edge_params': edge_params
        })
