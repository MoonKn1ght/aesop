...
if __name__ == '__main__':
    ...
    propagator = Propagator(window_t=1e-9, n_samples=2 ** 15, central_wl=1.55e-6)
    ...
    # target1 = input_laser.get_pulse_train(propagator.t, pulse_width=pulse_width * (p / q), rep_t=rep_t * (p / q),
    #                                       peak_power=peak_power * 1.0)
    # target2 = input_laser.get_pulse_train(propagator.t, pulse_width=pulse_width * (p / q), rep_t=rep_t * (p / q),
    #                                       peak_power=peak_power * 1.0)  # shift 0.5T
    # target2 = input_laser.get_pulse_train(propagator.t, pulse_width=pulse_width * (p / q), rep_t=rep_t * (p / q),
    #                                       peak_power=peak_power * 1.0, phase_shift=0.0)  # shift 0.5T

    # evaluator = PulseRepetition(propagator, target1, pulse_width=pulse_width, rep_t=rep_t, peak_power=peak_power)
